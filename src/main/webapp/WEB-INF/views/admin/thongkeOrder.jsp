				...
				<div class="col-lg-12 col-custom">
								...
								<c:forEach var="order" items="${orders}">
									<tr>
										<td class="pro-thumbnail"><a href="#"><img
												class="img-fluid"
												src="${pageContext.request.contextPath}/resources/images/bicycle/${order.bicycle.image}"
												alt="Product" /></a></td>
										<td class="pro-title"><a href="#">${order.bicycle.name}</a></td>
										<td class="pro-price"><span><fmt:formatNumber
													pattern="###,### VND" value="${order.amount}"
										...
										<td class="pro-subtotal"><span><fmt:formatNumber
													pattern="###,### VND"
													value="${order.amount*order.quantity}" type="currency" /></span></td>
										<c:if test="${order.status==true}">
											<td>Approved</td>
											<td class="pro-remove"><a href=""><i
													class="lnr lnr-cloud-check"></i></a></td>
										</c:if>
										<c:if test="${order.status==false}">
											<td>Pending</td>
											<td class="pro-remove"><a
												href="${pageContext.request.contextPath}/pages/my_account/userOrders/update/${order.id}.htm"><i
													class="lnr lnr-sync"></i></a> <span style="font-size: large;">
													/ </span> <a
												href="${pageContext.request.contextPath}/pages/my_account/userOrders/delete/${order.id}.htm"><i
					...
					<div
						...
						<div class="apply-coupon-wrapper">
							<a href="${pageContext.request.contextPath}/admin/chitiet.htm"
								class="btn flosun-button primary-btn rounded-0 black-btn">Trở lại trang quản lý</a>
						</div>
					</div>
				</div>
			</div>
								...
								<table class="table">
									<tr>
										...
										<td><span class="amount"><fmt:formatNumber
													pattern="###,### VND" value="${subTotal1}" type="currency" /></span></td>
									</tr>
									<tr>
										<td>Shipping</td>
										<td><span class="amount"><fmt:formatNumber
													pattern="###,### VND" value="${ship1}" type="currency" /></span></td>
									</tr>
									<tr class="total">
										<td>Total</td>
